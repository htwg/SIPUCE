 ...
 */
package de.fhkn.in.uce.turn.server.connectionhandling;
...
public final class HandleMessageTask implements Runnable {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(HandleMessageTask.class);
	 ...
	 */
	public HandleMessageTask(final Socket s,
	...
	public void run() {
		try {
			while (this.socket.isConnected() && !this.socket.isClosed()) {
				...
				LOGGER.debug("received message at local {}:{}",
	...
	private void handleMessage(final Message toHandle) {
		if (toHandle.isMethod(STUNMessageMethod.BINDING)) {
			...
			LOGGER.debug("Can not handle message with method {}",
					toHandle.getMessageMethod());
		}
	}

	private void handleBindingMessage(final Message toHandle)
			throws IOException {
		if (toHandle.isRequest()
				&& toHandle.isMethod(STUNMessageMethod.BINDING)) {
			LOGGER.debug("handling binding request");
			this.handleSimpleBindingRequest(toHandle);
		} else if (this.isPrimaryAddress()
				&& toHandle.hasAttribute(ChangeRequest.class)) {
			final InetSocketAddress remoteAddress = new InetSocketAddress(
					this.socket.getInetAddress(), this.socket.getPort());
			LOGGER.debug("handling message with change request attribute");
			this.handleMessageWithChangeRequestAttribute(toHandle,
					remoteAddress);
		}
	}

	private void handleSimpleBindingRequest(final Message toHandle)
			throws IOException {
		final Message response = toHandle.buildSuccessResponse();
		final OtherAddress otherAddress = new OtherAddress(
				this.secondaryAddress);
		response.addAttribute(otherAddress);
		final XorMappedAddress clientAddress = this
				.getPublicClientAddressAsAttribute(toHandle);
		response.addAttribute(clientAddress);
		response.writeTo(this.socket.getOutputStream());
	}

	private void handleMessageWithChangeRequestAttribute(
			final Message toHandle, final InetSocketAddress remoteAddress) {
		final ChangeRequest changeRequest = toHandle
				.getAttribute(ChangeRequest.class);
		LOGGER.debug("Getting indication with change request flag = {}",
				changeRequest.getFlag());
		switch (changeRequest.getFlag()) {
		case ChangeRequest.CHANGE_IP_AND_PORT:
			this.sendIndicationViaNewSocket(new InetSocketAddress(
					this.secondaryAddress.getAddress(), this.thirdPort),
					toHandle, remoteAddress);
			break;
		case ChangeRequest.CHANGE_PORT:
			this.sendIndicationViaNewSocket(new InetSocketAddress(
					this.primaryAddress.getAddress(), this.thirdPort),
					toHandle, remoteAddress);
			break;
		case ChangeRequest.FLAGS_NOT_SET:
			break;
		case ChangeRequest.CHANGE_IP:
			this.sendIndicationViaNewSocket(
					new InetSocketAddress(this.secondaryAddress.getAddress(),
	...
	private void sendIndicationViaNewSocket(
			final InetSocketAddress bindAddress, final Message message,
			final InetSocketAddress remoteAddress) {
		try {
			final Message indication = MessageStaticFactory
					.newSTUNMessageInstance(STUNMessageClass.INDICATION,
							STUNMessageMethod.BINDING);
			indication.addAttribute(this
					.getPublicClientAddressAsAttribute(message));
			final Socket newSocket = new Socket();
			newSocket.setReuseAddress(true);
			LOGGER.debug("Binding new socket to {}", bindAddress);
			newSocket.bind(bindAddress);
			LOGGER.debug("Connecting to {}", remoteAddress.toString());
			newSocket.connect(remoteAddress, TIMEOUT_IN_SECONDS);
			LOGGER.debug("Wrtiting indication");
			indication.writeTo(newSocket.getOutputStream());
			newSocket.close();
		} catch (IOException e) {
			LOGGER.error(
					"Connection to client not successfully established: {}",
					e.getMessage());
		}
	}

	private boolean isPrimaryAddress() {
		return this.socket.getLocalAddress().equals(
				this.primaryAddress.getAddress())
				&& this.socket.getLocalPort() == this.primaryAddress.getPort();
	}

	private XorMappedAddress getPublicClientAddressAsAttribute(
			final Message message) {
		XorMappedAddress clientAddress;
		final InetSocketAddress publicClientAddress = new InetSocketAddress(
				this.socket.getInetAddress(), this.socket.getPort());
		if (publicClientAddress.getAddress() instanceof Inet4Address) {
			clientAddress = new XorMappedAddress(publicClientAddress);
		} else {
			clientAddress = new XorMappedAddress(publicClientAddress,
					ByteBuffer.wrap(message.getHeader().getTransactionId())
							.getInt());
		}
