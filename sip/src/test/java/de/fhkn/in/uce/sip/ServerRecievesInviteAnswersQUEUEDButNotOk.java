...
public class ServerRecievesInviteAnswersQUEUEDButNotOk {
	...
	public void inviteNotAnsweredByUAS() {
		try {
			...
			System.out.println("TimeOut expected in 30s");
			synchronized (lock) {
				lock.wait();
			}
	...
	ISipManagerListener listenerServer = new ISipManagerListener() {
		...
		public void onInvite(final IMessage inviteMessage) {
			...
			try {
				sipManagerServer.sendQueued(sipUserServer, sipUserClient);
