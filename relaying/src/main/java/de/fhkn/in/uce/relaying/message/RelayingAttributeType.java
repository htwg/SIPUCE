...
public enum RelayingAttributeType implements AttributeType {
    ...
    private static final Map<Integer, RelayingAttributeType> INT_TO_ENUM = new HashMap<Integer, RelayingAttributeType>();

    static {
        for (RelayingAttributeType l : values()) {
        	INT_TO_ENUM.put(l.encoded, l);
        }
    }
    ...
    public static RelayingAttributeType fromEncoded(int encoded) {
        return INT_TO_ENUM.get(encoded);
    }
}
