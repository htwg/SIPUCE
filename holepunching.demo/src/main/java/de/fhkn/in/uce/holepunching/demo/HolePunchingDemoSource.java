 ...
 *
 */
public final class HolePunchingDemoSource 
{
	private HolePunchingDemoSource() {}
    ...
    public static void main(String[] args) throws UCEStunException, InterruptedException, IOException
    {
		...
		System.out.println("Starting HolePunchingDemoSource...");
		...
		ConnectionAuthenticator sourceAuth = new SourceConnectionAuthenticator(UUID.fromString("5445cec0-afa2-11e2-9e96-0800200c9a66"));
		
		holePuncher.establishHolePunchingConnection(privateEndpoint.getAddress(), privateEndpoint.getPort(), publicEndpoint.getAddress(), publicEndpoint.getPort(), sourceAuth);
		...
		chatExecutor.execute(new ReaderTask(socketToTarget.getOutputStream(), scanner));
		chatExecutor.execute(new PrinterTask(socketToTarget.getInputStream()));
